...
Task 4: Averaging Card Values
 ...
 - where your goal is to have the cards in your hand add up to a target value 
 - but in Black Joe the goal is to get the average of the card values to be 7. The average can be found by summing up all the card values and then dividing that sum by the number of cards in the hand.
...
def approx_average_is_average(hand):
    ...
    if (hand[0] + hand[-1]) / 2 == average:
        return True
    if hand[len(hand)//2] == average:
        return True
    return False

def average_even_is_average_odd(hand):
    return sum(hand)/len(hand) in hand
